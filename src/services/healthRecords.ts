...
export const uploadHealthRecord = async (file: File, userId: string): Promise<string> => {
  try {
    ...
    // Sanitize filename and create storage reference
    ...
    if (error instanceof Error) {
      if (error.message.includes('storage/unauthorized')) {
        throw new Error('Permission denied. Please check if you are properly authenticated and have the necessary permissions.');
      }
